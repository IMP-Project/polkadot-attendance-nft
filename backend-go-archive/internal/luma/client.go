...
// GetAttendee gets an attendee by ID - PRODUCTION ONLY
func (c *Client) GetAttendee(attendeeID string) (*models.Attendee, error) {
	return nil, fmt.Errorf("GetAttendee not implemented in production mode - use GetEventGuests instead")
}

// GetEvent gets an event by ID - PRODUCTION ONLY
func (c *Client) GetEvent(eventID string) (*models.Event, error) {
	return nil, fmt.Errorf("GetEvent not implemented in production mode - use FetchSingleEvent instead")
}
...
func (c *Client) ListEvents(apiKey string) ([]map[string]interface{}, error) {
	// Use your specific calendar ID to get events from "Polkadot Sam Luma Event" calendar
	url := "https://api.lu.ma/public/v1/calendar/list-events?calendar_api_id=cal-WW2SBTdgsTrJgG8&series_mode=events&pagination_limit=50&include_past_events=true"
