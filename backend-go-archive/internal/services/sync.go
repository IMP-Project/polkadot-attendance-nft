...
func (s *SyncService) SyncEventCheckIns(eventID string, apiKey string) error {
	...
	for i, guest := range guests {
		...
		if s.polkadotClient != nil {
			log.Printf("🚀 PRODUCTION MODE: Attempting real blockchain NFT mint for %s...", walletAddress)
			mintResult, err := s.polkadotClient.MintNFT(eventID, walletAddress, metadata)
			if err != nil {
				log.Printf("❌ PRODUCTION FAILURE: Blockchain minting failed for %s: %v", walletAddress, err)
				log.Printf("⚠️  No NFT record will be created - production standard only")
				continue // Skip this attendee completely if blockchain fails
			} else if !mintResult.Success {
				log.Printf("❌ PRODUCTION FAILURE: NFT minting returned false for %s: %s", walletAddress, mintResult.Error)
				log.Printf("⚠️  No NFT record will be created - production standard only")
				continue // Skip this attendee completely if blockchain fails
			} else if mintResult.TransactionHash == "" {
				log.Printf("❌ PRODUCTION FAILURE: No transaction hash returned for %s", walletAddress)
				log.Printf("⚠️  No NFT record will be created - production standard only")
				continue // Skip if no real transaction hash
			} else {
				log.Printf("✅ PRODUCTION SUCCESS: NFT minted with transaction hash: %s", mintResult.TransactionHash)
				transactionHash = mintResult.TransactionHash
			}
		} else {
			log.Printf("❌ PRODUCTION FAILURE: Polkadot client not available")
			log.Printf("⚠️  No NFT record will be created - production standard only")
			continue // Skip if no blockchain client
		}

		// PRODUCTION: Only create NFT database record AFTER successful blockchain minting
		nft := &models.NFT{
			EventID:         eventID,
			Owner:          walletAddress,
			Metadata:       metadata,
			TransactionHash: transactionHash, // Real blockchain transaction hash only
			Confirmed:      true,             // Mark as confirmed since we got a real tx hash
		}
		...
		log.Printf("✅ PRODUCTION SUCCESS: Complete NFT process for %s - TxHash: %s", walletAddress, transactionHash)
